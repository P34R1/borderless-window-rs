...
fn main() {
    let all_visible_windows = read_visible_windows();

    let options = all_visible_windows.keys().collect();
    ...
    if let Ok(choice) = ans {
        let hwnd = *all_visible_windows.get(choice).expect("getting hwnd");
        set_borderless_fullscreen(hwnd);
    }
}

fn set_borderless_fullscreen(hwnd: HWND) {
...
fn read_visible_windows() -> HashMap<String, HWND> {
    let mut visible_windows: HashMap<String, HWND> = HashMap::new();

    let windows_raw_ptr = &mut visible_windows as *mut HashMap<String, HWND> as minwindef::LPARAM;
